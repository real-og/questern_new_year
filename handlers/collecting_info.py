...
async def send_welcome(message: types.Message, state: FSMContext):
    if message.text.strip() == texts.adult_quest_btn:
        ...
        await aiotable.update_cell(message.from_user.id, 7, 'Взрослый')
        ...
        await aiotable.update_cell(message.from_user.id, 7, 'Ребенок')
        ...
        await aiotable.update_cell(message.from_user.id, 8, message.text.strip())
        ...
        await aiotable.update_cell(message.from_user.id, 6, 'Коробки')
