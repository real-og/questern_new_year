...
async def send_welcome(message: types.Message, state: FSMContext):
    if message.text == texts.candle_btn:
        await message.answer(texts.candle_to_lit, reply_markup=kb.hint_kb)
        await State.entering_lighters.set()
    elif message.text == texts.lamp_btn:
        await message.answer(texts.lamp_to_lit, reply_markup=kb.hint_kb)
        await State.entering_lighters.set()
    elif message.text == texts.other_btn:
        await message.answer(texts.other_to_lit, reply_markup=kb.hint_kb)
        await State.entering_lighters.set()
    else:
...
async def send_welcome(message: types.Message, state: FSMContext):
    if message.text == texts.get_hint_btn:
        await message.answer_sticker(r'CAACAgIAAxkBAAECuQhlmKYxb8GZKk06HShjFCyume58qwACkkAAAisayUglSPcJyZvgyTQE', reply_markup=kb.hint_kb)
    ...
    if is_correct_answer:
        ...
        await aiotable.update_cell(message.from_user.id, 7, '1961')
